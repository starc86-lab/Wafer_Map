...
| `chart_common` | 2D/3D 공통 (colormap, interp_method, show_circle, show_notch, notch_depth_mm, boundary_r_mm, radial_rings, radial_seg, show_scale_bar, chart_width, chart_height, decimals, edge_cut_mm, z_range_expand_pct, radial_line_width_mm, show_1d_radial, camera_distance) — `decimals`는 Summary 표 숫자 자릿수(0~3, 기본 2). `radial_line_width_mm`는 1D radial scan 자동 감지 직사각형 폭(기본 45mm, CMP 등 rotation symmetric 공정 라인 스캔). `z_range_expand_pct`는 공통 Z-scale 모드 range 확장 %(0=그대로, 50=1.5배, midpoint 고정). `show_1d_radial`는 1D Radial Graph 표시 (2D/3D 그래프와 Summary 표 사이에 (r,v) 산점도 + 스플라인). `camera_distance`는 Map Size (2D top view / 3D 공통, 400~800 step 10, 기본 550). **Settings UI 제거·개발자값 고정**: `show_circle`/`show_notch`/`show_scale_bar`는 항상 True, `notch_depth_mm=3.0`, `boundary_r_mm=153.0`, `radial_line_width_mm=45.0` 고정. |
