...
| `chart_2d` | 2D MAP 스타일 (colormap, contour, 측정점 표시 on/off 등) |
| `chart_3d` | 3D MAP 스타일 (colormap, 조명, 기본 카메라 등) |
...
### 예상 구조 (Profile Vision 참고)
```
Wafer Map/
├── app.py               ← GUI 진입점 (메인 윈도우)
├── main.py              ← 클립보드 파싱 / 데이터 전처리
├── core/
│   ├── themes.py        ← 컬러맵·팔레트·FONT_SIZES
│   ├── settings.py      ← settings.json 로드·저장
│   ├── stylesheet.py    ← 글로벌 QSS
│   └── runtime.py       ← 해상도 티어 판정
├── widgets/
│   ├── paste_area.py    ← Ctrl+V 입력 위젯 (계획)
│   ├── wafer_view.py    ← 2D heatmap 위젯
│   ├── wafer_view_3d.py ← 3D GLSurfacePlotItem 위젯
│   └── delta_dialog.py  ← DELTA 두 입력 매칭 (계획)
├── samples/             ← 라이브러리 비교 샘플 (확정 후 정리)
├── requirements.txt
└── settings.json
```

### 미정 사항 (논의 필요)
- 클립보드 파싱: 컬럼 자동 추론 vs 사용자 매핑 UI
- 다중 웨이퍼 입력 UX: 탭 / 리스트 / 좌우 비교 패널
- 3D 스케일링 정책 (Z 과장 비율 UI 제공?)
- 원 밖 좌표 처리: 제외 / 표시 / 경고
- settings.json 스키마 (Profile Vision 관례 재사용 가능 여부)
