...
| `chart_common` | 2D/3D 공통 (colormap, interp_method, grid_resolution, show_circle, show_notch) |
...
- **보간 캐시** `_interp_cache` + `_interp_key=(method, grid)` — 컬러맵·shading·점크기 등 변경 시 보간 재계산 없이 item만 재구성 (RBF 50~100ms 생략, refresh 3ms).
...
**웨이퍼 notch 표시**
- `chart_common.show_notch` (default on). 방향은 6시(3π/2) 고정, V자 폭 ±3°, 깊이 4mm 과장.
- `_boundary_xy(show_notch)` 헬퍼가 2D plot과 3D GLLinePlotItem이 공유.
