...
| `chart_common` | 2D/3D 공통 (colormap, interp_method, show_circle, show_notch, notch_depth_mm, boundary_r_mm, radial_rings, radial_seg, show_scale_bar, chart_width, chart_height, decimals, edge_cut_mm, z_range_expand_pct, radial_line_width_mm) — `decimals`는 Summary 표 숫자 자릿수(0~3, 기본 2). `radial_line_width_mm`는 1D radial scan 자동 감지 직사각형 폭(기본 45mm, CMP 등 rotation symmetric 공정 라인 스캔). `z_range_expand_pct`는 공통 Z-scale 모드 range 확장 %(0=그대로, 50=1.5배, midpoint 고정) |
