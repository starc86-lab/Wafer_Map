...
| `chart_common` | 2D/3D 공통 (colormap, interp_method, show_circle, show_notch, notch_depth_mm, boundary_r_mm, radial_rings, radial_seg, show_scale_bar, chart_width, chart_height, decimals, edge_cut_mm, z_range_expand_pct, radial_line_width_mm, show_1d_radial) — `decimals`는 Summary 표 숫자 자릿수(0~3, 기본 2). `radial_line_width_mm`는 1D radial scan 자동 감지 직사각형 폭(기본 45mm, CMP 등 rotation symmetric 공정 라인 스캔). `z_range_expand_pct`는 공통 Z-scale 모드 range 확장 %(0=그대로, 50=1.5배, midpoint 고정). `show_1d_radial`는 1D Radial Graph 표시 (2D/3D 그래프와 Summary 표 사이에 (r,v) 산점도 + 스플라인). |
...
- 구성: 실측 `(r_i, v_i)` 산점도 (검정 4px) + 1D 스플라인 실선 (`#555` 진회색, 1.5px). `RadialInterp` 재사용 — 메인 맵이 RBF/Radial 어느 경로든 무관
...
- 좌/우 축 폭 48px, 상/하 축 높이 22px 로 고정 — 좌측 Y 라벨 영역과 우측 테두리가 시각적 대칭. 전체 높이 135px
